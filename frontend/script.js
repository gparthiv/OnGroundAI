...
async function initializeDashboard() {
    try {
        ...
        addExecutionLog(' Error loading dashboard: ' + error.message, new Date());
    }
}
...
async function executeWorkflow(workflowData) {
    ...
    await simulateAgentExecution('DataIngestAgent', async () => {
        ...
        await delay(600);
        addExecutionLog('load_messages() → Success (3 items)', new Date());
        
        await delay(600);
        addExecutionLog('load_calendar() → Success (2 items)', new Date());
        
        await delay(600);
        addExecutionLog('load_tasks() → Success (2 items)', new Date());
        
        // Update UI
        document.getElementById('dataingest-status').textContent = '✓ Complete (1.8s)';
        document.getElementById('dataingest-status').classList.add('complete');
        document.getElementById('dataingest-tools').textContent = '3';
        
        const dataHtml = `
            <div class="agent-data-item">• load_messages → 3 items</div>
            <div class="agent-data-item">• load_calendar → 2 items</div>
            <div class="agent-data-item">• load_tasks → 2 items</div>
        `;
...
function displayToolRegistry(tools) {
    ...
    tools.forEach(tool => {
        ...
        toolItem.innerHTML = `
            ...
            <div class="tool-detail">Returns: ${tool.returns}</div>
            ${tool.last_used ? `<div class="tool-detail">Last Used: ${tool.last_used}</div>` : ''}
